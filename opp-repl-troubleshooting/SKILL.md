...
## §10. `make: *** No rule to make target 'makefiles'`

**Root cause:** `make_makefiles()` runs `make makefiles`, but
that target is only defined in an EXISTING top-level Makefile
generated by `opp_makemake`.  On a fresh project with no Makefile
yet, the target doesn't exist.

**Fix:** bootstrap with `opp_makemake` once, then use
`make_makefiles()` for every subsequent regeneration:

    # In a shell at the project root (or via subprocess from opp_repl):
    opp_makemake -f --deep -o <project_name>

    # Now `make makefiles` works:
    from opp_repl.simulation.build import make_makefiles
...
**Root cause:** `--meta:*` flags are consumed by **`make
makefiles`**, not by `opp_makemake` directly.  If you copied the
`makemake-options` string out of `.oppbuildspec` and pasted it
into a `opp_makemake` CLI call, the `--meta:*` flags don't
resolve.
...
`.oppbuildspec` where they belong — `make makefiles` translates
them correctly when it parses the file.
